...
scripts and, afterwards, proceeds to build and run the drake plan for the project. Information regarding the workflow, the functions used, and other useful functions can (eventually, but not yet) be found in the reference.Rmd file. 
...
source("./setup/libraries.R")       # loads packages and cleans environment 
source("./setup/functions.R")       # loads functions that we'll be using for analysis
source("./setup/setup.R")           # loads our model target and fixes some default settings
```


### Build and visualize our drake plan
```{r message = FALSE}
...
plan = drake_plan(
  ...
  raw_data= read_csv("./data/Key_indicator_districtwise.csv"),                        
...
### View the source code of any arbitrary drake plan
```{r}

# Print a formatted version of the code that built the plan w/o comments
drake_plan_source(plan)

# Print a tibble of each target and the corresponding code for each one by just typing in the plan name 
plan
```

### Peeking under the hood of the config object
```{r}

# Print tibbles of the elements of the graph 
explore = drake_graph_info(config)
explore$nodes # nodes
explore$edges # edges
```

### Viewing the full legend for vis_drake_network
```{r}

# Print an image of all possible node types for a drake network graph
visNetwork(nodes = legend_nodes())
```

### Subsetting our network graph
```{r}

# Drop imported objects and only view targets
vis_drake_graph(config, targets_only = TRUE) # still looks cluttered bc we have so much going on at first step

# Just show up-stream nodes
vis_drake_graph(config, from = "learners_rfor") # doesn't show our data, ctrl objects, resampling, etc. still good!
vis_drake_graph(config, from = "learners_glm")
vis_drake_graph(config, from = c("learners_rfor", "learners_xgb")) 
vis_drake_graph(config, from = "raw_data") # almost as big as original!

# Just show down-stream nodes
vis_drake_graph(config, from = c("mlr_task_train_data", "mlr_task_test_data"), mode = "in") # data flow 
vis_drake_graph(config, from = "metrics_glm", mode = "in") # probably the best way to show this network, per model


# Showa neighborhood of targets in both directions
vis_drake_graph(config, from = "results_glm", mode = "all", order = 2) # where order = <#> changes nodes out range

# Group and cluster our targets 
vis_drake_graph(config,
                group = "imported",
                clusters = "TRUE",
                from = "metrics_glm", 
                mode = "in")          # very uggo. unfortunately, no way yet to make multiple groups 
```



# Other notable features not covered here
# - drake can handle high-performance computing and parallelization, see:
#     https://ropenscilabs.github.io/drake-manual/hpc.html
# - a great example of a drake workflow implemented as package can be found here: 
#     https://github.com/tiernanmartin/drakepkg
# - the triggers that determine whether a drake target needs to be reran can be changed, see here:
#     https://ropenscilabs.github.io/drake-manual/hpc.html
# - for massive plans, drake can even be asked to build targets on its own, see:
#     https://ropenscilabs.github.io/drake-manual/plans.html#large-plans


# Presentation order of ops
# 1. change the metric to rmse and show how objects now outdated
# 2. add a comment to a function and show how nothing changed
# 3. change a fx and show how it changed
# 4. run through cells
